  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const avgMonthlySpending = Object.values(monthlyTotals).reduce((sum, total) => sum + total, 0) / (nonZeroMonths || 1);

        setFinancialData({
          balance: 8500.75, // Example static balance
          totalSpending,
          avgMonthlySpending,
          recentTransactions: transactions.filter((transaction) => transaction.amount < 0), // Filter for negative amounts
        });
      } catch (error) {
