...
exports.lambda_handler = async (event) => {
  try {
    ...
    const { public_token, bank, id, accounts = [] } = body;
    ...
    const institutionLogo = (await client.institutionsGetById({ institution_id: id, country_codes: ['US'] })).data.institution.logo;

    const newAccount = [{
      Bank: bank,
      Logo: institutionLogo,
      Balance: accountResponse.data.accounts[0].balances.current,
      Mask: accountResponse.data.accounts[0].mask,
      Name: accountResponse.data.accounts[0].name,
      accountID: accountResponse.data.accounts[0].account_id,
      accessToken,
    }];
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({ accessToken, accounts: updatedAccounts }),
    };
  } catch (error) {
