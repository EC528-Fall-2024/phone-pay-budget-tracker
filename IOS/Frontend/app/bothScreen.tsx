  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const monthlySpendingTotals = transactions.reduce((acc, transaction) => {
          if (transaction.amount < 0) { // Only count negative values
        ...
        const monthlyIncomeTotals = transactions.reduce((acc, transaction) => {
        ...
        const nonZeroSpendingMonths = Object.values(monthlySpendingTotals).filter(total => total > 0).length;
        const avgMonthlySpending = Object.values(monthlySpendingTotals).reduce((sum, total) => sum + total, 0) / (nonZeroSpendingMonths || 1);

        const nonZeroIncomeMonths = Object.values(monthlyIncomeTotals).filter(total => total > 0).length;
        const avgMonthlyIncome = Object.values(monthlyIncomeTotals).reduce((sum, total) => sum + total, 0) / (nonZeroIncomeMonths || 1);
      ...
      <ScrollView style={styles.container}>
        <View style={styles.summaryContainer}>
          <View style={[styles.summaryBox, styles.greenBackground]}>
            <Text style={styles.summaryLabel}>Total Spending This Month</Text>
        ...
        <View style={styles.summaryContainer}>
          <View style={[styles.summaryBox, styles.greenBackground]}>
            ...
            <Text style={styles.summaryValue}>${financialData.totalIncome.toFixed(2)}</Text>
          </View>
          <View style={[styles.summaryBox, styles.greenBackground]}>
            <Text style={styles.summaryLabel}>Average Monthly Income</Text>
            <Text style={styles.summaryValue}>${financialData.avgMonthlyIncome.toFixed(2)}</Text>
          </View>
        </View>
        ...
        <View style={styles.transactionsContainer}>
          ...
          {financialData.recentTransactions.map((transaction) => (
            <View key={transaction.id} style={styles.transactionItem}>
              ...
              <Text style={[styles.transactionAmount, transaction.amount < 0 ? styles.negative : styles.positive]}>
